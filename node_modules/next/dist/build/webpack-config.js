...
if(baseRes!==res){return;}if(res.match(/next[/\\]dist[/\\]next-server[/\\](?!lib[/\\]router[/\\]router)/)){return`commonjs ${request}`;}// Default pages have to be transpiled
...
...(_webpack.isWebpack5?{emitOnErrors:!dev}:{noEmitOnErrors:dev}),checkWasmTypes:false,nodeEnv:false,splitChunks:isServer?_webpack.isWebpack5&&!dev?{filename:'[name].js',// allow to split entrypoints
...
emacsLockfilePattern]},output:{...(_webpack.isWebpack5?{environment:{arrowFunction:false,bigIntLiteral:false,const:false,destructuring:false,dynamicImport:false,forOf:false,module:false}}:{}),// we must set publicPath to an empty value to override the default of
// auto which doesn't work in IE11
publicPath:'',path:isServer&&_webpack.isWebpack5&&!dev?_path.default.join(outputPath,'chunks'):outputPath,// On the server we don't use hashes
...
{}:{include:[dir,...babelIncludeRegexes]}),exclude:excludePath=>{if(babelIncludeRegexes.some(r=>r.test(excludePath))){return false;}return /node_modules/.test(excludePath);},use:hasReactRefresh?[require.resolve('@next/react-refresh-utils/loader'),defaultLoaders.babel]:defaultLoaders.babel},...(!config.images.disableStaticImages&&_webpack.isWebpack5?[{test:/\.(png|svg|jpg|jpeg|gif|webp|ico|bmp)$/i,loader:'next-image-loader',issuer:{not:_css.regexLikeCss},dependency:{not:['url']}}]:[])].filter(Boolean)},plugins:[hasReactRefresh&&new _ReactRefreshWebpackPlugin.default(_webpack.webpack),// Makes sure `Buffer` and `process` are polyfilled in client-side bundles (same behavior as webpack 4)
...
config.excludeDefaultMomentLocales&&new _webpack.webpack.IgnorePlugin({resourceRegExp:/^\.\/locale$/,contextRegExp:/moment$/}),...(dev?(()=>{// Even though require.cache is server only we have to clear assets from both compilations
...
version:`${"11.0.0"}|${configVars}`,cacheDirectory:_path.default.join(distDir,'cache','webpack')};// Adds `next.config.js` as a buildDependency when custom webpack config is provided
